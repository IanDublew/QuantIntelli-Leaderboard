document.addEventListener('DOMContentLoaded', () => {
    ...
    const supabaseKey = 'sb_publishable_T46p8tqFRqhSVN1SK9Ihyg__Spu9LWl';
    ...
    let detectedOddsSweetspotBuckets = null;
    ...
    async function fetchData() {
        try {
            ...
            renderOddsSweetspotPanel(allData);

            // KNN Nearest-Neighbour Edge Analysis (knn_analysis.js)
            if (window.KNNAnalysis) window.KNNAnalysis.render(allData);
    ...
    // ── ODDS SWEETSPOT ENGINE PANEL ──────────────────────────────────────────
    // Read-only analytical panel. Surfaces which odds buckets simultaneously satisfy:
    //   1. Win rate >= live statistical accuracy (auto-updates as predictions resolve)
    //   2. Positive expected value in that bucket
    // Stores qualifying buckets in detectedOddsSweetspotBuckets for slip builder tagging.
    // Does NOT affect EV analysis, tier scoring, or slip construction logic.
    function renderOddsSweetspotPanel(data) {
        const container = document.getElementById('odds-sweetspot-container');
        if (!container) return;

        // Recompute live statistical accuracy — same method as updateSummaryStats
        const valid1X2Types = ['home', 'draw', 'away'];
        const decidedWithStat = data.filter(item =>
            item.statistical &&
            valid1X2Types.includes(String(item.statistical).toLowerCase()) &&
            (item.state === 'won' || item.state === 'loss')
        );
        let correctStat = 0;
        decidedWithStat.forEach(item => {
            if (String(item.statistical).toLowerCase() === String(item.outcome).toLowerCase()) correctStat++;
        });
        const statAcc = decidedWithStat.length > 0 ? correctStat / decidedWithStat.length : 0.53;

        // Use existing full-dataset index builder (not walk-forward — we want the full picture)
        const oddsIndex = buildOddsPreferenceIndex(data);

        if (!oddsIndex) {
            container.innerHTML = `<p class="text-sm text-muted italic" style="padding:8px 0;">Not enough resolved data to run the Odds Sweetspot Engine.</p>`;
            detectedOddsSweetspotBuckets = null;
            return;
        }

        const MIN_SAMPLES = minSamplesPerBucket; // reuse existing constant (5)

        // Apply dual filter: WR >= statAcc AND EV > 0
        const processBuckets = (buckets) => buckets
            .filter(b => !showLowSampleBuckets ? b.totalPredictions >= MIN_SAMPLES : true)
            .map(b => ({
                ...b,
                isSweetspot: b.totalPredictions >= MIN_SAMPLES && b.winRate >= statAcc && b.ev > 0
            }));

        const homeBuckets = processBuckets(oddsIndex.home);
        const awayBuckets = processBuckets(oddsIndex.away);

        // Store qualified buckets — slip builder reads this for tagging
        detectedOddsSweetspotBuckets = {
            home: homeBuckets.filter(b => b.isSweetspot),
            away: awayBuckets.filter(b => b.isSweetspot)
        };

        const homeSweetspotCount = detectedOddsSweetspotBuckets.home.length;
        const awaySweetspotCount = detectedOddsSweetspotBuckets.away.length;
        const totalSweetspotCount = homeSweetspotCount + awaySweetspotCount;
        const thresholdPct = (statAcc * 100).toFixed(1);

        // ── Row renderer ──
        const renderBucketRows = (buckets) => {
            if (buckets.length === 0) {
                return `<p class="text-xs text-muted italic" style="padding:6px 0;">No qualifying data in this range.</p>`;
            }
            return buckets.map(b => {
                const wrPct = (b.winRate * 100);
                const evPct = (b.ev * 100);
                const wrStr = wrPct.toFixed(1) + '%';
                const evStr = (b.ev > 0 ? '+' : '') + evPct.toFixed(1) + '%';
                const avgStr = b.avgOdds > 0 ? b.avgOdds.toFixed(2) : 'N/A';

                const wrColor = wrPct >= statAcc * 100 ? 'var(--green)' : (wrPct >= 40 ? 'var(--amber)' : 'var(--red)');
                const evColor = b.ev > 0 ? 'var(--green)' : 'var(--red)';
                const barWidth = Math.min(wrPct, 100);

                // Dual-filter status pill
                const wrPass = b.totalPredictions >= MIN_SAMPLES && b.winRate >= statAcc;
                const evPass = b.ev > 0;

                const filterPillHtml = b.isSweetspot
                    ? `<span style="font-size:0.55rem;font-weight:700;letter-spacing:0.06em;color:var(--amber);background:rgba(255,179,0,0.1);border:1px solid rgba(255,179,0,0.3);border-radius:999px;padding:2px 7px;font-family:var(--font-mono);white-space:nowrap;">✓ SWEETSPOT</span>`
                    : `<span style="font-size:0.55rem;color:var(--text-muted);font-family:var(--font-mono);display:flex;gap:4px;align-items:center;">
                           ${!wrPass ? `<span style="color:var(--red);">✗WR</span>` : `<span style="color:var(--green);">✓WR</span>`}
                           ${!evPass ? `<span style="color:var(--red);">✗EV</span>` : `<span style="color:var(--green);">✓EV</span>`}
                       </span>`;

                const rowBorder = b.isSweetspot
                    ? 'border-color:rgba(255,179,0,0.35);background:linear-gradient(90deg,rgba(255,179,0,0.03) 0%,transparent 100%);'
                    : b.totalPredictions < MIN_SAMPLES
                        ? 'border-color:var(--border-subtle);opacity:0.5;'
                        : 'border-color:var(--border-subtle);';

                return `
                    <div style="display:grid;grid-template-columns:72px 1fr auto;align-items:center;gap:8px;padding:7px 10px;border-radius:var(--radius-sm);border:1px solid;margin-bottom:5px;${rowBorder}">
                        <div>
                            <div style="font-size:0.67rem;font-family:var(--font-mono);font-weight:600;color:var(--text-heading);white-space:nowrap;">${b.label}</div>
                            <div style="font-size:0.57rem;color:var(--text-muted);font-family:var(--font-mono);">n=${b.totalPredictions} · @${avgStr}</div>
                        </div>
                        <div style="display:flex;flex-direction:column;gap:3px;">
                            <div style="display:flex;align-items:center;gap:5px;">
                                <span style="font-size:0.58rem;color:var(--text-muted);font-family:var(--font-mono);width:14px;flex-shrink:0;">WR</span>
                                <div style="flex:1;height:3px;background:var(--bg-elevated);border-radius:2px;overflow:hidden;">
                                    <div style="height:100%;width:${barWidth}%;background:${wrColor};border-radius:2px;"></div>
                                </div>
                                <span style="font-size:0.62rem;font-family:var(--font-mono);color:${wrColor};width:34px;text-align:right;flex-shrink:0;">${wrStr}</span>
                            </div>
                            <div style="display:flex;align-items:center;gap:5px;">
                                <span style="font-size:0.58rem;color:var(--text-muted);font-family:var(--font-mono);width:14px;flex-shrink:0;">EV</span>
                                <div style="flex:1;height:3px;background:var(--bg-elevated);border-radius:2px;overflow:hidden;">
                                    <div style="height:100%;width:${Math.min(Math.abs(evPct) * 3, 100)}%;background:${evColor};border-radius:2px;"></div>
                                </div>
                                <span style="font-size:0.62rem;font-family:var(--font-mono);color:${evColor};width:34px;text-align:right;flex-shrink:0;">${evStr}</span>
                            </div>
                        </div>
                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;min-width:64px;">
                            ${filterPillHtml}
                        </div>
                    </div>`;
            }).join('');
        ...
        // ── Summary bar copy ──
        const summaryText = totalSweetspotCount > 0
            ? `${totalSweetspotCount} active sweetspot${totalSweetspotCount !== 1 ? 's' : ''} &nbsp;·&nbsp; ${homeSweetspotCount} home &nbsp;·&nbsp; ${awaySweetspotCount} away`
            : 'No odds buckets currently clear both filters';

        container.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:var(--bg-elevated);border:1px solid var(--border-subtle);border-radius:var(--radius-md);margin-bottom:14px;flex-wrap:wrap;gap:8px;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <i class="fas fa-crosshairs" style="color:${totalSweetspotCount > 0 ? 'var(--amber)' : 'var(--text-muted)'};font-size:0.9rem;"></i>
                    <div>
                        <div style="font-size:0.72rem;font-weight:600;color:var(--text-heading);font-family:var(--font-mono);">${summaryText}</div>
                        <div style="font-size:0.6rem;color:var(--text-muted);margin-top:2px;">
                            Threshold: WR &ge; ${thresholdPct}% (live stat accuracy) <span style="margin:0 4px;">·</span> EV &gt; 0%
                        </div>
                    </div>
                </div>
                ${totalSweetspotCount > 0
                ? `<span style="font-size:0.62rem;font-weight:700;color:var(--amber);background:rgba(255,179,0,0.08);border:1px solid rgba(255,179,0,0.25);padding:3px 10px;border-radius:999px;font-family:var(--font-mono);text-transform:uppercase;white-space:nowrap;">${totalSweetspotCount} PASS</span>`
                : `<span style="font-size:0.62rem;color:var(--text-muted);font-family:var(--font-mono);">0 PASS</span>`}
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:10px;">
                        <i class="fas fa-house-signal" style="color:var(--cyan);font-size:0.75rem;"></i>
                        <span style="font-size:0.72rem;font-weight:600;color:var(--cyan);font-family:var(--font-mono);">HOME ODDS</span>
                        <span style="font-size:0.6rem;color:${homeSweetspotCount > 0 ? 'var(--amber)' : 'var(--text-muted)'};font-family:var(--font-mono);">${homeSweetspotCount > 0 ? `(${homeSweetspotCount} pass)` : '(0 pass)'}</span>
                    </div>
                    ${renderBucketRows(homeBuckets)}
                </div>
                <div>
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:10px;">
                        <i class="fas fa-car-side" style="color:#f87171;font-size:0.75rem;"></i>
                        <span style="font-size:0.72rem;font-weight:600;color:#f87171;font-family:var(--font-mono);">AWAY ODDS</span>
                        <span style="font-size:0.6rem;color:${awaySweetspotCount > 0 ? 'var(--amber)' : 'var(--text-muted)'};font-family:var(--font-mono);">${awaySweetspotCount > 0 ? `(${awaySweetspotCount} pass)` : '(0 pass)'}</span>
                    </div>
                    ${renderBucketRows(awayBuckets)}
                </div>
            </div>

            <div style="margin-top:12px;padding:8px 12px;border-left:2px solid rgba(255,179,0,0.25);font-size:0.6rem;color:var(--text-muted);line-height:1.6;">
                <i class="fas fa-link" style="margin-right:5px;color:var(--amber);opacity:0.7;"></i>
                Qualifying buckets are tagged as <span style="color:var(--green);font-family:var(--font-mono);font-weight:600;">odds SS</span> on slip builder candidates. Threshold updates automatically as more predictions resolve.
            </div>
        `;
    }
    ...
    function renderSlipBuilder() {
        ...
        // Checks if the relevant odds side for this market falls in a detected odds sweetspot bucket.
        // Read-only — used purely for the candidate card tag. Does not affect tier or sorting.
        function isLegInOddsSweetspot(leg) {
            if (!detectedOddsSweetspotBuckets || !leg.rawOdds) return false;
            const check = (side, oddsVal) =>
                (detectedOddsSweetspotBuckets[side] || []).some(b => oddsVal >= b.min && oddsVal <= b.max);
            if (leg.market === 'DNB Home' || leg.market === '1X') return check('home', leg.rawOdds.home);
            if (leg.market === 'DNB Away' || leg.market === 'X2') return check('away', leg.rawOdds.away);
            if (leg.market === '12') return check('home', leg.rawOdds.home) || check('away', leg.rawOdds.away);
            return false;
        }
        ...
        if (slipStatus) {
            if (selectedLegs.length >= minLegs) {
                ...
                const oddsSsCount = selectedLegs.filter(leg => isLegInOddsSweetspot(leg)).length;
                const parts = [];
                if (awayEdgeCount > 0) parts.push(`${awayEdgeCount} away 2.21–2.50`);
                if (ssCount > 0) parts.push(`${ssCount} EV sweet spot`);
                if (oddsSsCount > 0) parts.push(`${oddsSsCount} odds SS`);
        ...
        if (selectedLegs.length === 0) {
            ...
            slipCandidatesList.innerHTML = selectedLegs.map(leg => {
                ...
                const isAwayEdge = awayPriority &&
                    ...
                    (leg.market === 'X2' || leg.market === 'DNB Away' || leg.market === '12');
                const isEvSweet = sweetSpotSet.has(getEvBucketLabel(evPct));
                const isOddsSweet = isLegInOddsSweetspot(leg);
                ...
                if (isOddsSweet) {
                    edgeTags.push(`<span style="font-size:0.58rem;color:var(--green);font-family:var(--font-mono);display:flex;align-items:center;gap:2px;"><i class="fas fa-crosshairs" style="font-size:0.5rem;"></i>odds SS</span>`);
                }
